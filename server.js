...
app.post('/addPayroll', (req, res) => {
    console.log(req.body);
    const { PayrollID, EmployeeID, BasicSalary, OvertimeHours, PayDate } = req.body;

    // Retrieve total BonusAmount from bonuses table
    const bonusQuery = `SELECT SUM(BonusAmount) AS TotalBonusAmount FROM bonuses WHERE EmployeeID = ?`;
    db.query(bonusQuery, [EmployeeID], (bonusErr, bonusResult) => {
        if (bonusErr) {
            console.error('Error retrieving bonus data:', bonusErr);
            res.status(500).send('Failed to add payroll');
            return;
        }
        const TotalBonusAmount = bonusResult[0].TotalBonusAmount || 0;

        // Retrieve total BenefitAmount from benefits table
        const benefitQuery = `SELECT SUM(BenefitAmount) AS TotalBenefitAmount FROM benefits WHERE EmployeeID = ?`;
        db.query(benefitQuery, [EmployeeID], (benefitErr, benefitResult) => {
            if (benefitErr) {
                console.error('Error retrieving benefit data:', benefitErr);
                res.status(500).send('Failed to add payroll');
                return;
            }
            const TotalBenefitAmount = benefitResult[0].TotalBenefitAmount || 0;

            // Retrieve total DeductionAmount from deductions table
            const deductionQuery = `SELECT SUM(DeductionAmount) AS TotalDeductionAmount FROM deductions WHERE EmployeeID = ?`;
            db.query(deductionQuery, [EmployeeID], (deductionErr, deductionResult) => {
                if (deductionErr) {
                    console.error('Error retrieving deduction data:', deductionErr);
                    res.status(500).send('Failed to add payroll');
                    return;
                }
                const TotalDeductionAmount = deductionResult[0].TotalDeductionAmount || 0;

                // Retrieve total TaxAmount from employeetax table
                const taxQuery = `SELECT SUM(TaxAmount) AS TotalTaxAmount FROM employeetax WHERE EmployeeID = ?`;
                db.query(taxQuery, [EmployeeID], (taxErr, taxResult) => {
                    if (taxErr) {
                        console.error('Error retrieving tax data:', taxErr);
                        res.status(500).send('Failed to add payroll');
                        return;
                    }
                    const TotalTaxAmount = taxResult[0].TotalTaxAmount || 0;

                    // Insert into payroll table with all retrieved amounts
                    const sql = `INSERT INTO payroll (PayrollID, EmployeeID, BasicSalary, OvertimeHours, BonusAmount, BenefitAmount, PayDate, DeductionAmount, TaxAmount) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;

                    db.query(sql, [PayrollID, EmployeeID, BasicSalary, OvertimeHours, TotalBonusAmount, TotalBenefitAmount, PayDate, TotalDeductionAmount, TotalTaxAmount], (err, result) => {
                        if (err) {
                        ...
                        res.status(201).send('Payroll added successfully');
                    });
                });
            });
        });
